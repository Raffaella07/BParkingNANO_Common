...
template<typename FITTER, typename LORENTZ_VEC, typename DAUGHTER_VEC>
inline double cos_theta_2D_star(const FITTER& fitter, const reco::BeamSpot &bs, const LORENTZ_VEC& p4, DAUGHTER_VEC& p4_daughter) {
  if(!fitter.success()) return -2;

  GlobalPoint point = fitter.fitted_vtx();
  
   ROOT::Math::Boost boost(p4.BoostToCM());
 // math::PtEtaPhiMLorentzVectorD dvec(p4_daughter.pt(),p4_daughter.eta(),p4_daughter.phi(),p4_daughter.M());
//  math::PtEtaPhiMLorentzVectorD hvec(p4.pt(),p4.eta(),p4.phi(),p4.M());
 // reco::Candidate* hnl = p4->clone();
 // reco::Candidate* daughter = p4_daughter->clone();
 // std::cout <<  "px dau" << p4_daughter.px() << "py dau " << p4_daughter.py()<< std::endl;
  //CenterOfMassBooster boost(*hnl);

 // std::cout << "cos2d star" << hnl->pt() << " " << daughter->pt() <<std::endl;
 // auto boost = p4.BoostToCM(); 
//  CLHEP::HepLorentzVector d_copy(p4_daughter.px(),p4_daughter.py(),p4_daughter.pz(),p4_daughter.E());
    TLorentzVector d_copy;
    d_copy.SetPtEtaPhiM(p4_daughter.pt(),p4_daughter.eta(),p4_daughter.phi(),p4_daughter.mass());
    TLorentzVector hnl;
    hnl.SetPtEtaPhiM(p4.pt(),p4.eta(),p4.phi(),p4.mass());
//  CLHEP::Hep3Vector b_copy(boost.x(),boost.y(),boost.z());
//  d_copy.Boost(boost.x(),boost.y(),boost.z());
  //d_copy.Boost(-boost);

  d_copy.Boost(-hnl.BoostVector());
  hnl.Boost(-hnl.BoostVector());
  auto bs_pos = bs.position();
  math::XYZVector delta(point.x() - bs_pos.x(), point.y() - bs_pos.y(),point.z() - bs_pos.z());
  math::XYZVector p(d_copy.Px(), d_copy.Py(),d_copy.Pz());
  double den = (delta.R() * p.R());
//  std::cout << "cos2d star" << delta.Dot(p)/den <<std::endl;
  return (den != 0.) ? delta.Dot(p)/den : -2;
}
