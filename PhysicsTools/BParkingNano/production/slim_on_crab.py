...
config.General.requestName = "privateHNL_slimmed_%s" % production_tag
config.General.workArea = 'crab_privateData_nano_Slimmed'
...
config.JobType.scriptExe = '/afs/cern.ch/work/r/ratramon/HNL/CommonFrame/CMSSW_10_2_15/src/PhysicsTools/BParkingNano/production/SlimExe.sh'
config.JobType.scriptArgs = ['job_input_file_list_1.txt']
#config.JobType.outputFiles = ['MiniAOD.root','GenSimAODSim_step1.log', 'GenSimAODSim_step2.log', 'FrameworkJobReport.xml', 'job.log']
config.JobType.inputFiles = ['FrameworkJobReport.xml',"HNL_select.py"]
...
config.Data.outputPrimaryDataset = 'private_BtoDMuN_SlimData'
config.Data.userInputFiles = open("SlimIn.txt").readlines() #implementing input through list of file: works wherever when path file is wired through xrootd  
